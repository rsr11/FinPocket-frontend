...
const Login = () => {
      ...
      return;}
       ...
       try {
          //  alert(email+" "+ password);
            const data = await api.post("/finPocket/api/auth/login",{
            ...
            if(data.status === 200){
              dispatch(UpdateLoggedIn({name:data?.data?.name ,email,isLoggedIn:true,monthlyIncome:data?.data?.MonthlyIncome}));
              setLoading(false);
  ...
  return (
    <main>
      <Header/>
     <section className='' > 
    <div className='flex justify-center flex-col bg-red-20 items-center h-[70vh]' >
    <h1 className={`text-2xl ${loading ? "hidden" : "block"} sm:text-3xl sm:mb-1 relative text-center mt-10 font-semibold`} >Login</h1>
      ...
      {/* <h1 className='absolute sm:left-24 left-2 text-xl sm:text-2xl top-1 sm:top-8 p-4' >FinPocket 🪙 </h1> */}
       ...
       {loading ? <Loader/> : 
       <form onSubmit={(e)=>{onLoginSubmit(e)}} className='flex bg-red-5 flex-col w-[80vw] lg:w-[30vw] md:w-[60vw] sm:w-[30vw] rounded-md shadow-2xl p-5' >
        ...
        <button disabled={true} className='py-2 cursor-not-allowed text-sm sm:text-base rounded-md m-auto border-2 w-[50%] font-semibold border-blue-500 text-black hover:bg-blue-500 hover:text-white active:border-blue-500 active:text-black' > Google </button>
        </section>   
